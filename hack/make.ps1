...
Function Get-UpstreamCommit() {
    Invoke-Expression "git fetch -q https://github.com/rumpl/bof.git refs/heads/master"
...
Function Validate-DCO($headCommit, $upstreamCommit) {
    ...
    if ($badCommits.Length -eq 0) {
        ...
        $e += "https://github.com/rumpl/bof/blob/master/CONTRIBUTING.md#sign-your-work`n"
        Throw $e
    }
}
...
Function Validate-PkgImports($headCommit, $upstreamCommit) {
    ...
    $badFiles=@(); $files | ForEach-Object{
        ...
        $imports = @() + $imports -NotMatch "^github.com/rumpl/bof/pkg/" `
                                  -NotMatch "^github.com/rumpl/bof/vendor" `
                                  -NotMatch "^github.com/rumpl/bof/internal" `
                                  -Match "^github.com/rumpl/bof" `
                                  -Replace "`n", ""
...
Function Run-UnitTests() {
    ...
    $testPath="./..."
    $goListCommand = "go list -e -f '{{if ne .Name """ + '\"github.com/rumpl/bof\"' + """}}{{.ImportPath}}{{end}}' $testPath"
    $pkgList = $(Invoke-Expression $goListCommand)
    if ($LASTEXITCODE -ne 0) { Throw "go list for unit tests failed" }
    $pkgList = $pkgList | Select-String -Pattern "github.com/rumpl/bof"
    $pkgList = $pkgList | Select-String -NotMatch "github.com/rumpl/bof/vendor"
    $pkgList = $pkgList | Select-String -NotMatch "github.com/rumpl/bof/man"
    $pkgList = $pkgList | Select-String -NotMatch "github.com/rumpl/bof/integration"
...
Try {
    ...
    $ldflags = "-X 'github.com/rumpl/bof/dockerversion.Version="+$dockerVersion+"'"
    $ldflags += " -X 'github.com/rumpl/bof/dockerversion.GitCommit="+$gitCommit+"'"
    $ldflags += " -X 'github.com/rumpl/bof/dockerversion.BuildTime="+$env:BUILDTIME+"'"
    $ldflags += " -X 'github.com/rumpl/bof/dockerversion.PlatformName="+$env:PLATFORM+"'"
    $ldflags += " -X 'github.com/rumpl/bof/dockerversion.ProductName="+$env:PRODUCT+"'"
    $ldflags += " -X 'github.com/rumpl/bof/dockerversion.DefaultProductLicense="+$env:DEFAULT_PRODUCT_LICENSE+"'"
